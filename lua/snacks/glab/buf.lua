...
---@class snacks.glab.Buf
---@field buf number
---@field opts snacks.glab.Config
---@field item snacks.glab.api.View
local M = {}
M.__index = M

---@class vim.var_accessor
---@field snacks_glab? { repo: string, type: string, iid: number }

---@type table<number, snacks.glab.Buf>
M.attached = {}
...
function M.new(buf, item)
  ...
  self.buf = buf
  self.item = item
  self.opts = vim.deepcopy(require("snacks.glab").config())
  self.opts.bo = vim.tbl_extend("force", {}, self.opts.bo or {}, {
    buftype = "acwrite",
    swapfile = false,
    filetype = "markdown.glab",
  })
  vim.b[buf].snacks_glab = {
    repo = item.repo,
    type = item.type,
  ...
  for name, km in pairs(self.opts.keys or {}) do
    if km ~= false then
      ...
      elseif type(rhs) == "string" and not Actions.actions[rhs] then
        Snacks.notify.error(("Invalid glab buffer keymap action `%s:%s`"):format(name, rhs))
      end
    end
  end
end
...
function M.attach(buf, item)
  ...
  if not item then
    ...
    local repo, type, iid = Item.from_uri(name)
    if not repo then
      Snacks.notify.error("Invalid glab:// buffer: " .. name)
      return
    end
    item = {
      repo = repo,
...
function M.setup()
  ...
  did_setup = true
  local group = vim.api.nvim_create_augroup("snacks.glab.buf", { clear = true })
  ...
  -- glab:// buffers are rendered views; they can't be written
